...
export function generateComponent(
  ...
  const props: Guidance['props'] = fact.props.map((p) => {
    ...
    return {
      ...
      description: axisValues
        ? `Closed set: ${axisValues.join(', ')}.`
        : p.name === 'className'
          ? 'Layout and positioning only. Never appearance — see RULES.md.'
          : todo(`what does \`${p.name}\` control on ${fact.name}?`),
    }
  })
  for (const p of props) {
    if (p.description.startsWith('TODO(human)')) {
      todos.push(`${fact.name}.${p.name}: ${p.description.replace('TODO(human): ', '')}`)
    }
  }
  ...
  if (!arch) {
    todos.push(`${fact.name} matches no known archetype — aliases, when-to-use and when-NOT-to-use need a human.`)
  }
  ...
  const guidance: Guidance = {
    name: fact.name,
    status: 'stable',
    aliases: arch?.aliases ?? [todo(`what else do your designers and engineers call ${fact.name}?`)],
    purpose: arch?.purpose ?? todo(`in one line, what is ${fact.name} for?`),
    useWhen: arch?.useWhen ?? [todo(`what conditions mean ${fact.name} is the right choice?`)],
    useInstead: arch?.useInstead ?? [
      { when: todo(`when is ${fact.name} the WRONG choice?`), use: todo('and what should be used instead?') },
    ],
    props,
    variants: fact.variants,
    defaultVariants: defaults,
    states,
    composition: arch?.composition ?? { note: todo(`what may ${fact.name} contain, and what may contain it?`) },
    content: arch?.content ?? {},
    a11y: arch?.a11y ?? {
      role: todo(`what ARIA role does ${fact.name} take?`),
      requiredLabel: todo('what labels it?'),
      keyboard: todo('what keys operate it?'),
    },
    tokens: [...new Set(consumes)],
    source: 'code',
    provenance: [
      `extracted from ${fact.file}`,
      `${Object.keys(fact.variants).length} variant axes recovered from the cva map`,
